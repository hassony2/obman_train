...
def visualize_contacts3d(ax, contact_infos, mesh_verts, alpha=0.1):
    ...
    for penetrating_vert, close_vert in zip(penetrating_verts,
                                            penetrating_close_verts):
        ax.plot(
            [penetrating_vert[0],
             close_vert[0]], [penetrating_vert[1], close_vert[1]],
            [penetrating_vert[2], close_vert[2]],
            c='r',
            alpha=alpha)
    for missed_vert, close_vert in zip(missed_verts, missed_close_verts):
        ax.plot(
            [missed_vert[0], close_vert[0]], [missed_vert[1], close_vert[1]],
            [missed_vert[2], close_vert[2]],
            c='b',
            alpha=alpha)


def visualize_contacts2d(ax,
                         contact_infos,
                         mesh_verts,
                         proj='z',
                         contact_alpha=0.5,
...
def visualize_batch(save_img_path,
                    sample,
                    results,
                    fig=None,
                    faces_right=None,
    ...
    if TransQueries.images in sample:
        ...
        raise ValueError(
            'Neither images nor objpoints3d present in sample for display')
    ...
    if 'joints_refine' in results:
        refine = True
        pred_batchjoints3d_refine = results[
            'joints_refine'].cpu().detach().numpy()
    else:
    ...
    if TransQueries.objpoints3d in sample:
        gt_batchobjpoints3d = sample[
            TransQueries.objpoints3d].detach().cpu().numpy()
    else:
    ...
    if 'objpoints3d_refine' in results:
        refine = True
        pred_batchobjpoints3d_refine = results[
            'objpoints3d_refine'].detach().cpu().numpy()
        if 'objfaces' in results:
            pred_objfaces = results['objfaces']
    else:
    ...
    if 'contact_info' in results:
        ...
        contact_infos = [
            ...
            for att, rep, cont in zip(attraction_masks, repulsion_masks,
                                      contact_points)
        ]
        has_contacts = True
    else:
    ...
    if 'center3d' in results:
        ...
        if pred_batchjoints3d is not None:
            pred_batchjoints3d = pred_batchjoints3d + center3d_preds[:, np.
                                                                     newaxis]
        if pred_batchverts3d is not None:
            pred_batchverts3d = pred_batchverts3d + center3d_preds[:,
                                                                   np.newaxis]
        if pred_batchobjpoints3d is not None:
            pred_batchobjpoints3d = pred_batchobjpoints3d + center3d_preds[:,
                                                                           np.
                                                                           newaxis]
    ...
    if TransQueries.center3d in sample and 'center3d' in results:
        ...
        if gt_batchobjpoints3d is not None:
            gt_batchobjpoints3d = gt_batchobjpoints3d + center3d_gt[:,
                                                                    np.newaxis]
    ...
    for row_idx in range(batch_nb):
        ...
        if images is not None:
            ...
            ax = fig.add_subplot(batch_nb * row_factor, col_nb,
                                 row_idx * row_factor * col_nb + 1)
            ax.imshow(input_img)
            if gt_batchjoints2d is not None:
                gt_joints2d = gt_batchjoints2d[row_idx]
                visualize_joints_2d(
                    ax, gt_joints2d, joint_idxs=False, alpha=0.5)
        ...
        if refine:
            ...
            pred_objpoints3d_refine = get_row(pred_batchobjpoints3d_refine,
                                              row_idx)
        ...
        if refine:
            ax = fig.add_subplot(
                batch_nb * row_factor,
                col_nb, (row_idx * row_factor + 1) * col_nb + 2,
                projection='3d')
            add_hand_obj_meshes(
                ax,
                sample,
        ...
        if refine:
            ax = fig.add_subplot(batch_nb * row_factor, col_nb,
                                 (row_idx * row_factor + 1) * col_nb + 3)
            add_joints_proj(ax, gt_joints3d, pred_joints3d_refine, proj='z')
            add_scatter_proj(
                ax, gt_objpoints3d, pred_objpoints3d_refine, proj='z')
            ax.invert_yaxis()
        ...
        if refine:
            ax = fig.add_subplot(batch_nb * row_factor, col_nb,
                                 (row_idx * row_factor + 1) * col_nb + 4)
            add_joints_proj(ax, gt_joints3d, pred_joints3d_refine, proj='y')
            add_scatter_proj(
                ax, gt_objpoints3d, pred_objpoints3d_refine, proj='y')
        ...
        if refine:
            ax = fig.add_subplot(batch_nb * row_factor, col_nb,
                                 (row_idx * row_factor + 1) * col_nb + 5)
            add_joints_proj(ax, gt_joints3d, pred_joints3d_refine, proj='x')
            add_scatter_proj(
                ax, gt_objpoints3d, pred_objpoints3d_refine, proj='x')
        if has_segm:
            ax = fig.add_subplot(batch_nb * row_factor, col_nb,
                                 row_idx * row_factor * col_nb + 6)
...
def add_scatter_proj(ax, gt_objpoints3d, pred_objpoints3d, proj='z'):
    ...
    if gt_objpoints3d is not None:
        ax.scatter(
            gt_objpoints3d[:, proj_1],
            gt_objpoints3d[:, proj_2],
            c='g',
            alpha=0.1,
            s=1)
    ax.set_aspect('equal')
...
def add_joints_proj(ax,
    ...
    if gt_keypoints is not None:
        visualize_joints_2d(
            ax,
            np.stack(
                [gt_keypoints[:, proj_1], gt_keypoints[:, proj_2]], axis=1),
            alpha=0.2,
    ...
    if pred_keypoints is not None:
        visualize_joints_2d(
            ax,
            np.stack(
                [pred_keypoints[:, proj_1], pred_keypoints[:, proj_2]],
                axis=1),
            joint_idxs=joint_idxs)
    ax.set_aspect('equal')
...
def save_pck_img(thresholds, pck_values, auc_all, save_pck_file, overlay=None):
    ...
    markersize = 16
    plt.plot(
        thresholds, pck_values, 'ro-', markersize=markersize, label='Ours')

    # Relative 3D for 12 sequences of stereohands
    if overlay == 'stereo_all':
        plt.title('Stereo dataset (12 seq.)', fontsize=40)
        gan_thresh = [
            20.2020, 22.2222, 24.2424, 26.2626, 28.2828, 30.3030, 32.3232,
            34.3434, 36.3636, 38.3838, 40.4040, 42.4242, 44.4444, 46.4646,
            48.4848, 50.5051
        ]

        gan_accuracies = [
            0.4416, 0.4772, 0.5101, 0.5410, 0.5699, 0.5968, 0.6212, 0.6445,
            0.6660, 0.6858, 0.7049, 0.7229, 0.7394, 0.7550, 0.7697, 0.7835
        ]
        plt.plot(
            gan_thresh,
            gan_accuracies,
    ...
    # Relative 3D for 2 sequences of stereohands
        ...
        gan_thresh = [
            19.1919, 22.2222, 25.2525, 28.2828, 31.3131, 34.3434, 37.3737,
            40.4040, 43.4343, 46.4646, 49.4949
        ]
        gan_accuracies_wo = [
            0.7031, 0.7323, 0.7586, 0.7831, 0.8056, 0.8249, 0.8424, 0.8586,
            0.8728, 0.8859, 0.8972
        ]
        gan_accuracies_w = [
            0.8713, 0.9035, 0.9271, 0.9446, 0.9574, 0.9670, 0.9741, 0.9795,
            0.9833, 0.9867, 0.9895
        ]
        plt.plot(
            gan_thresh,
            gan_accuracies_wo,
        ...
        plt.plot(
            ...
            label='Ganerated w')
        zimmerman_thresh = [
            21.0526315789474, 23.6842105263158, 26.3157894736842,
            28.9473684210526, 31.5789473684211, 34.2105263157895,
            36.8421052631579, 39.4736842105263, 42.1052631578947,
            44.7368421052632, 47.3684210526316, 50
        ]
        zimmerman_accs = [
            0.869888888888889, 0.896873015873016, 0.916849206349206,
            0.932142857142857, 0.943507936507937, 0.952753968253968,
            0.959904761904762, 0.966047619047619, 0.971595238095238,
            0.976547619047619, 0.980174603174603, 0.983277777777778
        ]
        plt.plot(
            zimmerman_thresh,
            zimmerman_accs,
        ...
        chpr_accs = [
            0.565789473684211, 0.717105263157895, 0.822368421052632,
            0.881578947368421, 0.914473684210526, 0.9375, 0.960526315789474
        ]
        plt.plot(
            chpr_thresh, chpr_accs, 'mD-', markersize=markersize, label='CHPR')

    else:
        plt.title(
            'auc in [{},{}]: {}'.format(thresholds[0], thresholds[-1],
                                        auc_all),
            fontsize=40)
    plt.ylim(0, 1)
