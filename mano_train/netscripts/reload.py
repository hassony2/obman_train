...
from mano_train.networks.branches.contactloss import compute_contact_loss, batch_pairwise_dist
...
def get_samples_score_interval(sorted_samples,
                               sorted_losses,
                               interval=[0.95, 1],
                               reverse=False):
    assert interval[0] >= 0 and interval[0] <= 1, 'bounds of interval should be in [0, 1], got lower bound {}'.format(
        interval[0])
    assert interval[1] >= 0 and interval[1] <= 1, 'bounds of interval should be in [0, 1], got upper bound {}'.format(
        interval[1])
    assert interval[0] < interval[1], 'Lower bound {} should be lower then upper bound {}'.format(
...
def display_top_middle_worse(loader,
                             model,
                             max_samples=500,
                             max_displays=5,
                             mano_faces=None,
...
def show_meshes(loader,
                ...
                get_emd=False,
                show_losses=[
                    'mano_verts3d', 'mano_joints3d', 'atlas_objpoints3d',
                    'atlas_scale3d', 'atlas_trans3d', 'final_chamfer_loss'
                ]):
    """
    ...
    for sample_idx, sample in enumerate(loader):
        ...
        for model_idx, model in enumerate(models):
            with torch.no_grad():
                ...
                if get_emd:
                    emd_vals = batch_emd(sample[TransQueries.objpoints3d], results['objpoints3d'])
            ...
            if len(loss_vals) == 1:
                ...
                for loss_val in loss_vals[1:]:
                    print_str = print_str + '{}: {} --> {} ({:.2f}%)'.format(
                        loss_name, loss_vals[0], loss_val, 100 *
                        (loss_val - loss_vals[0]) / loss_vals[0])
            print(print_str)
...
def render_mesh(sample,
    ...
    if save:
        ...
        save_hand_path = os.path.join(save_obj_folder,
                                      '{:08d}_hand.obj'.format(sample_idx))
        ...
        if pred_points is not None:
            ...
            save_path = os.path.join(save_pkl_folder,
                                     'mesh_penetr_{:04d}.pkl'.format(sample_idx))
            save_meshes_dict(save_path, pred_points, pred_faces, pred_verts,
                             mano_faces)
            save_obj_path = os.path.join(save_obj_folder,
                                         '{:08d}_obj.obj'.format(sample_idx))
    ...
    if render:
        ...
        if show_contacts:
            ...
            penetrating_verts = torch.Tensor(pred_verts)[all_penetr_masks[
                0]].cpu().numpy()
            penetrating_close_verts = all_close_matches[0][all_penetr_masks[
                0]].cpu().numpy()
            missed_verts = torch.Tensor(pred_verts)[all_missed_masks[
                0]].cpu().numpy()
            missed_close_verts = all_close_matches[0][all_missed_masks[
                0]].cpu().numpy()
            ...
            scene_children = scene_children + attraction_lines_children + repulsion_lines_children
        if show_gt:
