...
def get_dataset(
        ...
        meta={},
        max_queries=[
            TransQueries.affinetrans, TransQueries.images,
            TransQueries.verts3d, TransQueries.center3d, TransQueries.joints3d,
            TransQueries.objpoints3d, TransQueries.camintrs, BaseQueries.sides
        ],
        use_cache=True,
        limit_size=None):
    if dat_name == 'obman':
        ...
        pose_dataset = yanademo.YanaDemo(
            version=meta['version'], side=meta['side'])
    ...
    if limit_size is not None:
        if len(dataset) < limit_size:
            warnings.warn(
                'limit size {} < dataset size {}, working with full dataset'.
                format(limit_size, len(dataset)))
        else:
            warnings.warn('Working wth subset of {} of size {}'.format(
                dat_name, limit_size))
