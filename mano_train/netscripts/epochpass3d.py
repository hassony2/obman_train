...
def epoch_pass(loader,
    ...
    if train:
        ...
        save_img_folder = os.path.join(save_path, 'images', 'train',
                                       'epoch_{}'.format(epoch))
    else:
        model.eval()
        save_img_folder = os.path.join(save_path, 'images', 'val',
                                       'epoch_{}'.format(epoch))
        if save_results:
            save_results_folder = os.path.join(save_path, 'save_results',
                                               'val', 'epoch_{}'.format(epoch))
    ...
    for batch_idx, (sample) in enumerate(loader):
        ...
        if train:
            ...
            if inspect_weights:
                inspect_loss_names = [
                    'atlas_trans3d', 'atlas_objpoints3d', 'mano_verts3d',
                    'mano_shape', 'atlas_edge_regul'
                ]
                ...
                for inspect_loss_name in inspect_loss_names:
                    ...
                    if inspect_loss_name in model_losses:
                        ...
                        if loss_val is not None:
                            ...
                            print(inspect_loss_name,
                                  torch.norm(features.grad).item())
        ...
        save_img_path = os.path.join(save_img_folder,
                                     'img_{:06d}.png'.format(batch_idx))
    ...
    (epe_mean_all, _, epe_median_all, auc_all, pck_curve_all,
     thresholds) = evaluator.get_measures(0, 50, 20)
