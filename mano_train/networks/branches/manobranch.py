...
class ManoBranch(nn.Module):
    ...
    def forward(self,
        ...
        if use_stereoshape:
            ...
            assert is_rights.sum(
            ) == 0, 'When stereoshape is used only left hands expected'
        elif self.use_shape:
        ...
        if self.adapt_skeleton:
            if len(joints_left) != 0:
                joints_left = self.left_skeleton_reg(
                    joints_left.permute(0, 2, 1)).permute(0, 2, 1)
            if len(joints_right) != 0:
                joints_right = self.right_skeleton_reg(
                    joints_right.permute(0, 2, 1)).permute(0, 2, 1)
...
class ManoLoss():
    ...
    def compute_loss(self, preds, target):
        ...
        if TransQueries.verts3d in target and self.lambda_verts:
            verts3d_loss = torch_f.mse_loss(preds['verts'],
                                            target[TransQueries.verts3d])
            ...
            if self.normalize_hand:
                ...
                for link in links:
                    for joint_idx, n_joint_idx in zip(link[:-1], link[1:]):
                        ...
                        print('({}, {}) :{}'.format(joint_idx, n_joint_idx,
                                                    torch.mean(bone_ratio)))
        ...
        if BaseQueries.hand_pcas in target and self.lambda_pca:
            pca_loss = torch_f.mse_loss(preds['pcas'],
                                        target[BaseQueries.hand_pcas])
