...
def batch_mesh_contains_points(
    ...
    dets = torch.bmm(
        v0v1.view(batch_points_size, 1, 3),
        pvec.view(batch_points_size, 3, 1)).view(batch_size, triangle_nb)
    ...
    u_val = torch.bmm(
        ...
        pvec.view(batch_size * tvec.shape[1], 3, 1)).view(
            batch_size, tvec.shape[1]) * invdet
    ...
    v_val = torch.bmm(
        ...
        qvec.view(batch_size * qvec.shape[1], 3, 1)).view(
            batch_size, qvec.shape[1]) * invdet
...
def batch_mesh_contains_point(ray_origin, obj_triangles, tol_thresh=0.000001):
    ...
    direction = torch.Tensor([0.4395064455, 0.617598629942,
                              0.652231566745]).cuda()
    batch_direction = direction.view(1, 1, 3).expand(batch_size, v0v2.shape[1],
                                                     3)
    pvec = torch.cross(batch_direction, v0v2, dim=2)
    batch_points_size = batch_size * v0v1.shape[1]
    dets = torch.bmm(
        v0v1.view(batch_points_size, 1, 3),
        pvec.view(batch_points_size, 3, 1)).view(batch_size, v0v1.shape[1])
