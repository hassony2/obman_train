...
class AtlasBranch(nn.Module):
    def __init__(
        ...
        if self.predict_trans:
            self.decode_trans = torch.nn.Sequential(
                torch.nn.Linear(self.bottleneck_size, int(self.bottleneck_size / 2)),
                torch.nn.ReLU(),
        ...
        if mode == "sphere":
            test_mesh = trimesh.creation.icosphere(subdivisions=inference_ico_divisions)
        ...
        self.test_verts = torch.Tensor(np.array(test_verts).astype(np.float32)).cuda()
    ...
    def forward(self, img_features):
        ...
        rand_grid = img_features.new_empty((img_features.size(0), 3, self.points_nb))
        ...
        img_features = img_features.unsqueeze(2).repeat(1, 1, rand_grid.size(2))
    ...
    def forward_inference(self, img_features, separate_encoder_features=None):
        ...
        if self.separate_encoder:
            ...
            dec_img_features = img_features.unsqueeze(2).repeat(1, 1, test_grid.size(2))
...
def edge_loss(edges, faces):
    ...
    # edge_lengths_A = torch.norm(edges_B - edges_A, dim=2)
    # edge_lengths_B = torch.norm(edges_C - edges_B, dim=2)
    # edge_lengths_C = torch.norm(edges_A - edges_C, dim=2)
    # all_edges = torch.cat([edge_lengths_C, edge_lengths_B, edge_lengths_A], dim=1)
    # edge_loss = torch_f.mse_loss(all_edges, all_edges.mean(1, keepdim=True).repeat(1, all_edges.shape[1]))
    ...
    all_edges = torch.cat([edge_lengths_C, edge_lengths_B, edge_lengths_A], dim=1)
    mean_edge_size = all_edges.mean(1, keepdim=True).repeat(1, all_edges.shape[1])
...
class AtlasLoss:
    def __init__(
        ...
        if lambda_laplacian:
            self.laplacian_loss = LaplacianLoss(laplacian_faces, laplacian_verts)
    ...
    def compute_loss(self, preds, target):
        ...
        if (
            ...
            if (
                ...
                # obj_centroids = (target[TransQueries.objpoints3d].max(
                #     1)[0] + target[TransQueries.objpoints3d].min(1)[0]) / 2
                trans3d_loss = torch_f.mse_loss(preds["objtrans"], obj_centroids)
                ...
                if "objscale" in preds:
                    # obj_scales = torch.norm(centered_objpoints3d, 2,
                    #                         2).max(1)[0] / 100
                    obj_scales = torch.norm(centered_objpoints3d, 2, 2).max(1)[0]
                ...
                atlas_losses["final_{}_loss".format(self.atlas_loss)] = sym_final_loss
                final_loss = (
