...
class AtlasBranch(nn.Module):
    def __init__(self,
        ...
        if self.predict_trans:
            self.decode_trans = torch.nn.Sequential(
                torch.nn.Linear(self.bottleneck_size,
                                int(self.bottleneck_size / 2)),
                torch.nn.ReLU(),
    ...
    def forward(self, img_features):
        ...
        rand_grid = img_features.new_empty((img_features.size(0), 3,
                                            self.points_nb))
...
class AtlasLoss():
    ...
    def compute_loss(self, preds, target):
        atlas_losses = {}
        if (TransQueries.objpoints3d in target and (self.lambda_atlas or self.final_lambda_atlas)) or (TransQueries.center3d in target and self.trans_weight):
            # Translation + centered object is predicted
            if 'objtrans' in preds and TransQueries.objpoints3d in target and (
                    'objpointscentered3d' in preds):
                ...
                trans3d_loss = torch_f.mse_loss(preds['objtrans'],
                                                obj_centroids)
                atlas_losses['atlas_trans3d'] = trans3d_loss
                centered_objpoints3d = (target[TransQueries.objpoints3d] -
                ...
                if 'objscale' in preds:
                    ...
                    obj_scales = torch.norm(centered_objpoints3d, 2,
                                            2).max(1)[0]
                    scale3d_loss = torch_f.mse_loss(preds['objscale'],
                                                    obj_scales.unsqueeze(1))
            ...
            if self.edge_regul_lambda is not None and (self.edge_regul_lambda >
                                                       0):
