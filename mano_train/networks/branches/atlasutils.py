...
def create_disk(radius_divides=10, radius=1):
    perimeter = 2 * np.pi
    all_points = []
    for radius_sec in range(radius_divides + 1):
        # Uniformly sample le radius
        radius = radius_sec / radius_divides
        perimeter = 2 * np.pi * radius

        # Sample matching radius circle with ~ same edge length
        perim_sections = perimeter * radius_divides
        theta = np.linspace(0, np.pi * 2, perim_sections)
        points = np.column_stack((np.sin(theta), np.cos(theta))) * radius
...
def create_plane(side_points=10):
    points = []
    for i in range(0, int(side_points + 1)):
        for j in range(0, int(side_points + 1)):
            points.append([i / side_points, j / side_points])
