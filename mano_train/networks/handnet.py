...
class HandNet(nn.Module):
    def __init__(self,
        ...
        if self.absolute_lambda or mano_lambda_joints2d:
            self.absolute_branch = AbsoluteBranch(
                base_neurons=[img_feature_size + 3,
        ...
        if mano_lambda_verts or mano_lambda_joints3d or mano_lambda_joints2d or mano_lambda_pca:
    ...
    def forward(self,
        ...
        if self.absolute_lambda and TransQueries.center3d in sample and (
        ...
        if predict_center:
            ...
            absolute_input = torch.cat(
                (focals.unsqueeze(1), u_0.unsqueeze(1), v_0.unsqueeze(1),
                 features),
                dim=1)
            ...
            if not no_loss and supervise_center:
                absolute_loss = torch_f.mse_loss(
                    pred_center3d, sample[TransQueries.center3d]).view(1)
                if total_loss is None:
                    total_loss = absolute_loss
                else:
                    total_loss += self.absolute_lambda * absolute_loss
                losses['absolute_loss'] = absolute_loss
        if (TransQueries.joints3d in sample.keys()
                or TransQueries.verts3d in sample.keys()
                or (TransQueries.joints2d in sample.keys() and TransQueries.camintrs in sample.keys())
            ) and BaseQueries.sides in sample.keys() and self.mano_lambdas:
            if sample['root'] == 'palm':
                root_palm = True
            else:
                root_palm = False
            ...
            if self.lambda_joints2d and predict_center:
                non_hom = torch.bmm( sample[TransQueries.camintrs], (mano_results['joints'] + results['center3d'].unsqueeze(1)).permute(0, 2, 1))
                proj_joints2d = (non_hom / non_hom[:, 2].unsqueeze(1))[:, :2, :].permute(0, 2, 1)
        ...
        predict_atlas = TransQueries.objpoints3d in sample.keys() and (self.atlas_lambda or self.atlas_final_lambda)
        if predict_atlas:
            if self.atlas_mesh:
                if self.adapt_atlas_decoder:
            ...
            if self.need_collisions:
                ...
                if not no_loss:
                    if TransQueries.verts3d in sample and TransQueries.objpoints3d in sample:
                    ...
                    contact_loss = self.contact_lambda * attraction_loss + self.collision_lambda * penetration_loss
