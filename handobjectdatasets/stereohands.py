...
from handobjectdatasets.queries import BaseQueries, TransQueries, get_trans_queries
...
class StereoHands():
    ...
    def _get_json(self, prefix):
        coord2d_path = os.path.join(self.coord2d_folder,
                                    '{}.txt'.format(prefix))
        annots = np.loadtxt(coord2d_path)
        # annots = json.load(open(coord2d_path))
        return annots

    def _extract_joints(self, annots):
        # joints = np.asarray(json_annots['coords'])
        # joints = np.concatenate([[joints[20, :], joints[0:20, :]])
        return annots

    def get_joints_3d(self, idx):
        prefix = self.prefixes[idx]
        coord3d_path = os.path.join(self.coord3d_folder,
                                    '{}.txt'.format(prefix))
        annots = np.loadtxt(coord3d_path)
        return annots
    ...
    def load_dataset(self):
        ...
        if os.path.exists(cache_path) and self.use_cache:
            ...
            reorder_idx = np.array([
                0, 17, 18, 19, 20, 13, 14, 15, 16, 9, 10, 11, 12, 5, 6, 7, 8,
                1, 2, 3, 4
            ])
            if not self.gt_detections:
            ...
            for sequence in sorted(self.sequences):
                ...
                label_path = os.path.join(self.label_folder,
                                          '{}_BB.mat'.format(sequence))
                rawmat = loadmat(label_path)
                annots = rawmat['handPara'].transpose(2, 1, 0)
                for i in range(1500):
                    left_img_path = os.path.join(self.rgb_folder, sequence,
                                                 self.left_template.format(i))
