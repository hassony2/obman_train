...
class FHBHands():
    ...
    def get_objpoints2d(self, idx):
        ...
        hom_2d = np.array(self.cam_intr).dot(
            objpoints3d.transpose()).transpose()
...
def get_skeletons(skeleton_root, subjects_info):
    ...
    for subject, samples in tqdm(subjects_info.items(), desc='subj'):
        for (action, seq_idx) in tqdm(samples, desc='sample'):
            skeleton_path = os.path.join(skeleton_root, subject, action,
                                         seq_idx, 'skeleton.txt')
            ...
            if len(skeleton_vals):
                assert np.all(
                    ...
                    skeleton_path)
                skelet_dict[subject][(action,
                                      seq_idx)] = skeleton_vals[:, 1:].reshape(
