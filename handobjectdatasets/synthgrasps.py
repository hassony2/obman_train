...
class SynthGrasps():
    def __init__(self,
        ...
        if mode == 'all' and not self.override_scale:
            self.all_queries = [
                ...
                BaseQueries.sides, BaseQueries.segms, BaseQueries.verts3d,
                BaseQueries.hand_pcas, BaseQueries.hand_poses,
                BaseQueries.camintrs, BaseQueries.depth
            ]
        ...
        # Object info
        self.shapenet_template = os.path.join(shapenet_root, '{}/{}/models/model_normalized.pkl')
        self.load_dataset()
    ...
    def load_dataset(self):
        ...
        if self.filter_class_ids is not None:
            class_id_suffix = '_'.join(sorted(self.filter_class_ids))
        else:
            class_id_suffix = ''

        cache_path = os.path.join(
            self.cache_folder, '{}_{}_v{}_mode_{}_{}.pkl'.format(
        ...
        if os.path.exists(cache_path) and self.use_cache:
            ...
            print('sample_nb : {}'.format(
                np.unique(
                    ...
                    axis=0).shape))
            print('grasp_nb : {}'.format(np.unique(grasp_ids, axis=0).shape))
