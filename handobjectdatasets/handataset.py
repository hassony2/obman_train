...
class HandDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 black_padding=False,
                 rot3d=True,
                 as_obj_only=False):  # TODO back to TRUE !!!
        """
        Args:
    ...
    def get_sample(self, idx, query=None):
        ...
        # Get 2D hand joints
        if (TransQueries.joints2d in query) or (
                TransQueries.images in query) or TransQueries.depth in query:
        ...
        if BaseQueries.segms in query or TransQueries.segms in query:
            ...
            if TransQueries.segms in query:
                segm = handutils.transform_img(segm, affinetrans,
        ...
        if ((BaseQueries.joints3d in query) or (TransQueries.joints3d in query)
            ...
            obj_only = (TransQueries.objverts3d in query
                        or TransQueries.objpoints3d in query) and no_query_in(
                            center3d_queries,
                            self.pose_dataset.all_queries) or self.as_obj_only
            if not obj_only:
                if one_query_in([TransQueries.objpoints3d] +
                                [TransQueries.objverts3d] + center3d_queries,
                                query):
                    ...
                    if self.train:
                        joints3d = rot_mat.dot(joints3d.transpose(
                            1, 0)).transpose()
                    # Compute 3D center
                    ...
                    if TransQueries.joints3d in query and (self.center_idx is
                                                           not None):
        ...
        if TransQueries.objpoints3d in query and (
            ...
            obj_verts3d = points3d
              ...
              or TransQueries.objverts3d in query) and (
            ...
            obj_verts3d = vertexsample.points_from_mesh(
                ...
                obj_verts3d,
                show_cloud=False,
                vertex_nb=self.point_nb).astype(np.float32)
        ...
        if TransQueries.images in query:
            ...
            # Transform and crop
            img = handutils.transform_img(img, affinetrans,
                                          [self.inp_res, self.inp_res])
        ...
        if TransQueries.depth in query:
            # Transform and crop
            depth = cv2.warpAffine(depth, affinetrans[:2],
                                   (self.inp_res, self.inp_res))
    ...
    def visualize_original(self, idx, joint_idxs=False):
        queries = [
            BaseQueries.sides, BaseQueries.images, BaseQueries.joints2d,
            BaseQueries.objpoints2d, BaseQueries.camintrs,
            BaseQueries.objverts3d, BaseQueries.objfaces, BaseQueries.depth
        ]
        sample_queries = []
        ...
        fig = plt.figure()
        if BaseQueries.depth in sample:
        ...
        if BaseQueries.depth in sample:
            ...
            depth = sample[BaseQueries.depth]
            # Scatter hand joints on image
            viz2d.visualize_joints_2d(
                ax, joints, joint_idxs=False, links=self.pose_dataset.links)
            ax.imshow(depth)

    #  if BaseQueries.camintrs in sample and (
    #          BaseQueries.objfaces in sample
    #  ) and BaseQueries.objverts3d in sample:
    #      verts = torch.from_numpy(
    #          sample[BaseQueries.objverts3d]).unsqueeze(0).cuda()
    #      verts = verts / 1000
    #      faces = torch.from_numpy(
    #          sample[BaseQueries.objfaces]).unsqueeze(0).cuda().int()
    #      objtexture = torch.Tensor([0, 1, 0]).expand(
    #          1, faces.shape[1], 2, 2, 2, 3).cuda()
    #      renderer = Renderer(orig_size=256, light=False)
    #      camintr = sample[BaseQueries.camintrs].astype(np.float32)

    #      render = renderer.forward(
    #          verts,
    #          faces,
    #          objtexture,
    #          K=torch.from_numpy(camintr).unsqueeze(0).cuda())
    #      ax.imshow(render[0][1].cpu().detach().numpy()[::-1], alpha=0.5)
        plt.show()
    ...
    def visualize_3d_proj(self, idx, joint_idxs=False):
        queries = [
            BaseQueries.sides, BaseQueries.images, TransQueries.joints3d,
            TransQueries.images, TransQueries.objpoints3d,
            TransQueries.verts3d, BaseQueries.joints2d
        ]
    ...
    def visualize_3d_transformed(self, idx, joint_idxs=False):
        queries = [
            BaseQueries.images, BaseQueries.sides, TransQueries.joints3d,
            TransQueries.images, TransQueries.objpoints3d,
            TransQueries.verts3d, BaseQueries.joints2d
        ]
    ...
    def visualize_transformed(self, idx, joint_idxs=False):
        queries = [
            BaseQueries.images, BaseQueries.joints2d, BaseQueries.sides,
            BaseQueries.depth, TransQueries.depth, TransQueries.images,
            TransQueries.joints2d, TransQueries.objverts3d,
            BaseQueries.objfaces, TransQueries.camintrs, TransQueries.center3d,
        ...
        if TransQueries.joints2d in sample:
            ...
            viz2d.visualize_joints_2d(
                ...
                joint_idxs=joint_idxs,
                links=self.pose_dataset.links)
        if TransQueries.camintrs in sample and (
                TransQueries.objverts3d in sample
        ) and BaseQueries.objfaces in sample:
            verts = torch.from_numpy(
                sample[TransQueries.objverts3d]).unsqueeze(0).cuda()
            center3d = torch.from_numpy(
                sample[TransQueries.center3d]).cuda().unsqueeze(0)
            ...
            objtexture = torch.Tensor([0, 1, 0]).expand(
                1, faces.shape[1], 2, 2, 2, 3).cuda()
            # renderer = Renderer(orig_size=256, light=False)
            # new_camintr = sample[TransQueries.camintrs].astype(np.float32)
            # render = renderer.forward(
            #     verts,
            #     faces,
            #     objtexture,
            #     K=torch.from_numpy(new_camintr).unsqueeze(0).cuda())
            # ax.imshow(render[0][1].cpu().detach().numpy()[::-1], alpha=0.5)
        if TransQueries.depth in sample:
            ax = fig.add_subplot(122)
            ax.imshow(sample[TransQueries.depth])
