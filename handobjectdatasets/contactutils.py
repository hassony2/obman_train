...
def load_contacts(
        save_contact_paths='/sequoia/data1/yhasson/code/pose_3d/handobjectdatasets/misc/contacts/contact_zones_synthgrasps_10_0.01.pkl',
        display=False):
    with open(save_contact_paths, 'rb') as p_f:
        contact_data = pickle.load(p_f)
    hand_verts = contact_data['verts']
    if display:
        colors = ['#f04e36', '#f36e27', ['#f3d430'], ['#1eb19d'], ['#ed1683'], ['#37bad6']]
        ...
        for zone_idx, zone_vert_idxs in contact_data['contact_zones'].items():
            ax.scatter(hand_verts[zone_vert_idxs, idx_1],
                        hand_verts[zone_vert_idxs, idx_2], hand_verts[zone_vert_idxs, idx_3], s=100, c=colors[zone_idx])
